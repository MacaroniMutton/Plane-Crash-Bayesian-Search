...
# Compute cumulative probability density along trajectories
cumulative_density = np.zeros_like(rd_distribution)
for trajectory in rd_trajectories:
    for point in trajectory:
        lat_idx = np.searchsorted(y_grid.flatten(), point[0]) - 1
        lon_idx = np.searchsorted(x_grid.flatten(), point[1]) - 1
        cumulative_density[lat_idx, lon_idx] += np.exp(kde.score_samples([[point[0], point[1]]]))

lon_grid_2d, lat_grid_2d = np.meshgrid(x_grid, y_grid)

# Visualize cumulative density
plt.figure(figsize=(10, 10))
plt.contourf(lon_grid_2d, lat_grid_2d, cumulative_density, cmap='viridis', levels=50)
plt.colorbar(label='Cumulative Probability Density')
plt.scatter(recovery_positions[:, 0], recovery_positions[:, 1], color='red', label='Recovery Position', zorder=10)
plt.title('Cumulative Reverse Drift Probability Density')
