...
recovery_position_mean = np.mean(recovery_positions, axis=0)  # Mean recovery position
distances = np.linalg.norm(all_positions - recovery_position_mean, axis=1)
...
kde.fit(all_positions, sample_weight=weights)
...
# Plotting trajectories and RD distribution side by side
...
# Plot RD distribution with weighted KDE
...
axs[1].set_title('Weighted Reverse Drift Probability Density (RD Distribution)')
