...
# Define grid parameters for the reduced area
grid_resolution = (96, 96)  # Reduced grid resolution
grid_extent = ((4.25, 5.75), (4.25, 5.75))  # Reduced grid extent in latitude and longitude (degrees)
...
# Function to simulate reverse drift trajectory within the reduced area
def simulate_reverse_drift(recovery_position, recovery_time, grid_extent,
    ...
    for t in range(recovery_time, 0, -time_step):
        ...
        # Append new position to trajectory if still within grid extent
        if (grid_extent[0][0] <= current_position[0] <= grid_extent[0][1] and
            grid_extent[1][0] <= current_position[1] <= grid_extent[1][1]):
            trajectory.append(current_position.copy())
        else:
            break  # Stop simulation if position moves outside of grid extent
...
for i in range(len(recovery_positions)):
    ...
    # Simulate reverse drift trajectory within reduced area
...
# Compute KDE for RD distribution within the reduced area
...
# Generate grid points for RD distribution visualization within reduced area
x_grid, y_grid = np.meshgrid(np.linspace(grid_extent[0][0], grid_extent[0][1], grid_resolution[0]),
                              np.linspace(grid_extent[1][0], grid_extent[1][1], grid_resolution[1]))
grid_points = np.vstack([x_grid.ravel(), y_grid.ravel()]).T

# Evaluate KDE model on grid points to get RD distribution within reduced area
log_density = kde.score_samples(grid_points)
rd_distribution = np.exp(log_density).reshape(x_grid.shape)

# Visualize RD distribution within the reduced area
plt.figure(figsize=(8, 8))
contour = plt.contourf(x_grid, y_grid, rd_distribution, cmap='viridis', levels=50)
...
plt.title('Reverse Drift Probability Density (RD Distribution) - Reduced Area')
plt.xlabel('Latitude')
