...
for i in range(len(recovery_positions)):
    ...
    rd_trajectories.extend(rd_trajectory)

rd_trajectories = np.array(rd_trajectories)

# Calculate distance weights based on trajectory points
recovery_position = np.mean(recovery_positions, axis=0)  # Mean recovery position
distances = np.linalg.norm(rd_trajectories - recovery_position, axis=1)
max_distance = np.max(distances)
weights = 1 - (distances / max_distance)  # Inverse linear weighting

# Compute KDE with weighted samples
kde = KernelDensity(bandwidth=0.1)
kde.fit(rd_trajectories, sample_weight=weights)
...
# Visualize RD distribution with weighted KDE
plt.figure(figsize=(10, 10))
...
plt.title('Weighted Reverse Drift Probability Density (RD Distribution)')
plt.xlabel('Latitude')
plt.ylabel('Longitude')
plt.legend()
plt.grid(True)
