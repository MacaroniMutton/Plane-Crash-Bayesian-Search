        ...
        for i in range(self.rows):
                for j in range(self.columns):
                    ...
                    if j==self.plane_coords[0] and i==self.plane_coords[1]:
                        self.grid[f"{j};{i}"] = {"tot_prob": self.gridprod[i, j], "p": self.grid1[i,j], "q":self.grid2[i,j], "lat_range": lat_range, "lng_range": lng_range, "plane_present": True}
                    else:
                        self.grid[f"{j};{i}"] = {"tot_prob": self.gridprod[i, j], "p": self.grid1[i,j], "q":self.grid2[i,j], "lat_range": lat_range, "lng_range": lng_range, "plane_present": False}
